(() => {
  ...
  if (!stage) return;
  const copy = {
    kk:{concept:'КОНЦЕПТ / 01',drag:'Сүйреп бұрыңыз',rotate:'Айналдыру',pause:'Тоқтату',kind:'Дизайн прототипі',colors:['Түнгі көк','Піл сүйегі','Кәріптас'],names:['DOS / LINE 01','DOS / LINE 02','DOS / SUN 03'],types:['Ацетат пішіні','Жұқа металл пішіні','Күннен қорғайтын пішін'],fallback:'Бұл құрылғы 3D көзілдірікті көрсете алмайды.',canvas:'Көзілдіріктің 3D көрінісі. Бұру үшін сүйреңіз немесе бағыт пернелерін басыңыз. Бастапқы көрініс үшін Home.'},
    ru:{concept:'КОНЦЕПТ / 01',drag:'Перетаскивайте для вращения',rotate:'Вращать',pause:'Остановить',kind:'Дизайн-прототип',colors:['Ночной синий','Слоновая кость','Янтарь'],names:['DOS / LINE 01','DOS / LINE 02','DOS / SUN 03'],types:['Ацетатная форма','Тонкая металлическая форма','Солнцезащитная форма'],fallback:'Это устройство не может показать 3D-очки.',canvas:'3D-модель очков. Перетаскивайте или используйте стрелки для вращения. Home возвращает исходный ракурс.'},
    en:{concept:'CONCEPT / 01',drag:'Drag to rotate',rotate:'Rotate',pause:'Pause',kind:'Design prototype',colors:['Midnight blue','Ivory','Amber'],names:['DOS / LINE 01','DOS / LINE 02','DOS / SUN 03'],types:['Acetate frame','Slim metal frame','Sunglasses frame'],fallback:'This device cannot display the 3D glasses.',canvas:'3D view of glasses. Drag or use arrow keys to rotate. Press Home to reset the view.'}
  };
  ...
  let spin=false,shape=1,color=0,explosion=0,currentExplosion=0;
  ...
  stage.insertAdjacentHTML('beforeend',`<div class="disassembly-controls"><button class="explode-toggle" aria-pressed="false"></button><input class="explode-slider" type="range" min="0" max="100" value="0"><output class="explode-readout">0%</output></div><svg class="callout-lines" aria-hidden="true">${partKeys.map(k=>`<line data-line="${k}"></line><circle r="3" data-dot="${k}"></circle>`).join('')}</svg><div class="part-hotspots">${partKeys.map((k,i)=>`<span class="part-hotspot" data-part="${k}"><span>${String(i+1).padStart(2,'0')}</span><b></b></span>`).join('')}</div>`);
  const canvas = stage.querySelector('canvas');
  const refreshCopy=()=>{
    if (!canvas.isConnected) {
      const message=stage.querySelector('.scene-fallback-message');
      if(message)message.textContent=words().fallback;
      return;
    }
  ...
  const fallback=()=>{stage.innerHTML='';const msg=document.createElement('p');msg.className='scene-fallback-message';msg.textContent=words().fallback;stage.append(msg);};
